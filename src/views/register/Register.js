...
const Register = (props) => {
    ...
    const {autenticado, registerUser,error} = authContext;
  ...
  return (
    <div>
      ...
      <Col span={12} >
         ...
         className='login_col login_pre'>
          
         <p className='label'>Register</p>
            <Input placeholder='name' type='text' name='name' value={name} onChange={handleChange}></Input>
            <br /> 
            ...
            <br /> 
            <Input placeholder='Repeat Password' type='password' name='confirm' value={confirm} onChange={handleChange}></Input>
            <br />
              {alerta ? <Error message={alerta} /> :null}
            <Button className='button2' type='primary' onClick={onSubmit}>Register</Button>
              <br /> <br />
            <Link to={'/login'} className='link'>do you already have an account? , log in</Link>
         </form>
